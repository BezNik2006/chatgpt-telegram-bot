...
GPT_5_MODELS = ("gpt-5", "gpt-5-mini", "gpt-5-nano")
...
class OpenAIHelper:
    ...
    async def __common_get_chat_response_vision(self, chat_id: int, content: list, stream=False):
        ...
        try:
            ...
            max_tokens_str = 'max_completion_tokens' if self.config['vision_model'] in O_MODELS or self.config['vision_model'] in GPT_5_MODELS else 'max_tokens'
            common_args = {
                'model': self.config['vision_model'],
                'messages': self.conversations[chat_id][:-1] + [message],
                'temperature': self.config['temperature'],
                'n': 1, # several choices is not implemented yet
                max_tokens_str: self.config['vision_max_tokens'],
                'presence_penalty': self.config['presence_penalty'],
                'frequency_penalty': self.config['frequency_penalty'],
                'stream': stream
            }
